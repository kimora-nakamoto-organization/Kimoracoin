...
`test/functional/test_framework/test_framework.py:KimoraTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/kimora_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_kimorad = config["components"].getboolean("ENABLE_KMRD")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/kimora-nakamoto-organization/Kimoracoin/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/kimora-nakamoto-organization/Kimoracoin/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_cli and enable_kimorad):
        print("No functional tests to run. Wallet, utils, and kimorad must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if kimorad is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "kimorad"]) is not None:
            print("%sWARNING!%s There is already a kimorad process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "KMRD" not in os.environ:
        os.environ["KMRD"] = build_dir + '/src/kimorad' + exeext
        os.environ["KMRCLI"] = build_dir + '/src/kimora-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `kimora-cli help` (`rpc_interface.txt`).
