...
"""Kimora P2P network half-a-node.

This python code was modified from ArtForz' public domain  half-a-node, as
found in the mini-node branch of http://github.com/jgarzik/pynode.

NodeConn: an object which manages p2p connectivity to a kimora node
...
# Objects that map to kimorad objects, which can be serialized/deserialized
...
class CTransaction():
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in kimorad
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_headers():
    ...
    def deserialize(self, f):
        # comment in kimorad indicates these should be deserialized as blocks
...
class NodeConnCB():
    """Callback and helper functions for P2P connection to a kimorad node.
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=NODE_NETWORK, send_version=True):
        ...
        logger.info('Connecting to Kimora Node: %s:%d' % (self.dstaddr, self.dstport))
