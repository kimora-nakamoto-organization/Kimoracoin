...
The RPM spec file provided here is for Kimora-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://kimora.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Kimora git repository](https://github.com/kimora-nakamoto-organization/Kimoracoin) but are not part of
...
Currently only `Source10` is used, for the example `kimora.conf` file.
...
in the kimora git repository. At present only an SVG version of the Kimora
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the kimora
...
needed when 0.12.1 is released, a proper fix is already in the Kimora git
master branch.
...
idea to build Kimora Core against a static version of that library compiled
...
The `kimora.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `kimora-server` package. At this point, most RPM based
...
The `kimora-server` package should have SELinux support. How to properly do
...
Kimora-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/kimora/). He has been
packaging Kimora for Fedora at least since 2012.
...
4. I renamed the `kimora` package that contains the Qt GUI to `kimora-core` as
that appears to be how the general population refers to it, in contrast to
`kimora-xt` or `kimora-classic`. I wanted to make sure the general population
