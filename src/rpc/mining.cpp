...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated kimora to.\n"
...
UniValue getmininginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"errors\": \"...\"            (string) DEPRECATED. Same as warnings. Only shown when kimorad is started with -deprecatedrpc=getmininginfo\n"
            "}\n"
            "\nExamples:\n"
...
// NOTE: Unlike wallet RPC (which use KMR values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "    https://github.com/kimora/bips/blob/master/bip-0022.mediawiki\n"
            "    https://github.com/kimora/bips/blob/master/bip-0023.mediawiki\n"
            "    https://github.com/kimora/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
            "    https://github.com/kimora/bips/blob/master/bip-0145.mediawiki\n"
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Kimora is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Kimora is downloading blocks...");
...
//    const struct VBDeploymentInfo& segwit_info = VersionBitsDeploymentInfo[Consensus::DEPLOYMENT_SEGWIT];
    ...
    // Kimoracoin community bounty fund
    if (nHeight + 1 <= DEV_FUND_UNTIL) {
...
UniValue submitblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
        throw std::runtime_error(
            ...
            "See https://en.kimora.it/wiki/BIP_0022 for full specification.\n"
...
UniValue estimatefee(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("estimatefee")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "estimatefee is deprecated and will be fully removed in v0.17. "
            "To use estimatefee in v0.16, restart kimorad with -deprecatedrpc=estimatefee.\n"
...
UniValue getgenerate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "It is set with the command line argument -gen (or " + std::string(KMR_CONF_FILENAME) + " setting gen)\n"
...
UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    int numCores = GenerateKimoras(fGenerate, nGenProcLimit, Params());
