...
void CWallet::AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapAssetCoins, bool fGetKMR, bool fGetAssets, bool fOnlySafe, const CCoinControl *coinControl, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t& nMaximumCount, const int& nMinDepth, const int& nMaxDepth) const {
    vCoins.clear();

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (fGetKMR) { // Looking for KMR Tx OutPoints Only
                    if (fKMRLimitHit) // We hit our limit
                        continue;

                    // We only want KMR OutPoints. Don't include Asset OutPoints
                    if (isAssetScript)
                        continue;
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListAssets() const
{
    ...
    // https://github.com/kimora-nakamoto-organization/Kimoracoin/pull/10340, but that change has been
    // postponed until after https://github.com/kimora-nakamoto-organization/Kimoracoin/pull/10244 to
                                   ...
                                   const AssetType& assetType, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-kimora-address
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    if (IsDust(txout, ::dustRelayFee) && !IsScriptTransferAsset(recipient.scriptPubKey)) /** KMR START */ /** KMR END */
                    {
 ...
 * https://kimoratalk.org/?topic=54527, or
 * https://github.com/kimora-nakamoto-organization/Kimoracoin/pull/1393.
 */
