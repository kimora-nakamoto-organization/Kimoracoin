...
#include "kimoraaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Kimora address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
#endif
    widget->setValidator(new KimoraAddressEntryValidator(parent));
    widget->setCheckValidator(new KimoraAddressCheckValidator(parent));
}
...
bool parseKimoraURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no kimora: URI
    if(!uri.isValid() || uri.scheme() != QString("kimora"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!KimoraUnits::parse(KimoraUnits::KMR, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseKimoraURI(QString uri, SendCoinsRecipient *out)
{
    // Convert kimora:// to kimora:
    //
    //    Cannot handle this later, because kimora:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("kimora://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "kimora:");
    }
    QUrl uriInstance(uri);
    return parseKimoraURI(uriInstance, out);
}

QString formatKimoraURI(const SendCoinsRecipient &info)
{
    QString ret = QString("kimora:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(KimoraUnits::format(KimoraUnits::KMR, info.amount, false, KimoraUnits::separatorNever));
        paramCount++;
    }
...
bool openKimoraConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(KMR_CONF_FILENAME);
    ...
    /* Open kimora.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Kimora.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Kimora (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Kimora (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Kimora*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "kimora.desktop";
    return GetAutostartDir() / strprintf("kimora-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a kimora.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Kimora\n";
        else
            optionFile << strprintf("Name=Kimora (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the kimora app
...
bool GetStartOnSystemStartup()
{
    CFURLRef kimoraAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (kimoraAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, kimoraAppUrl);

    CFRelease(kimoraAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add kimora app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, kimoraAppUrl, nullptr, nullptr);
    }
