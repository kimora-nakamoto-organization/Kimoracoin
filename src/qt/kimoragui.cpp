...
const std::string KimoraGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString KimoraGUI::DEFAULT_WALLET = "~Default";

KimoraGUI::KimoraGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void KimoraGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Kimora address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and kimora: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to KMR addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Kimora addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Kimora addresses"));
    ...
    openAction->setStatusTip(tr("Open a kimora: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Kimora command-line options").arg(tr(PACKAGE_NAME)));
...
void KimoraGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/kimoracointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* kimoraLabelLayout = new QVBoxLayout(toolbarWidget);
        kimoraLabelLayout->addWidget(label);
        kimoraLabelLayout->addWidget(toolbar);
        kimoraLabelLayout->setDirection(QBoxLayout::TopToBottom);
        kimoraLabelLayout->addStretch(1);
...
void KimoraGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool KimoraGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool KimoraGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void KimoraGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void KimoraGUI::setWalletActionsEnabled(bool enabled)
{
...
void KimoraGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void KimoraGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void KimoraGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void KimoraGUI::showDebugWindow()
{
...
void KimoraGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void KimoraGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void KimoraGUI::openClicked()
{
...
void KimoraGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void KimoraGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void KimoraGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void KimoraGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void KimoraGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void KimoraGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** KMR START */
void KimoraGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void KimoraGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void KimoraGUI::gotoManageAssetsPage()
{
...
void KimoraGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Kimora network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void KimoraGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void KimoraGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void KimoraGUI::updateHeadersSyncProgressLabel()
{
...
void KimoraGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void KimoraGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Kimora"); // default title
...
void KimoraGUI::changeEvent(QEvent *e)
{
...
void KimoraGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void KimoraGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void KimoraGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "KMR")
        msg += tr("Amount: %1\n").arg(KimoraUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(KimoraUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void KimoraGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void KimoraGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool KimoraGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool KimoraGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void KimoraGUI::setHDStatus(int hdEnabled)
{
...
void KimoraGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void KimoraGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void KimoraGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void KimoraGUI::detectShutdown()
{
...
void KimoraGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void KimoraGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void KimoraGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(KimoraGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void KimoraGUI::subscribeToCoreSignals()
{
...
void KimoraGUI::unsubscribeFromCoreSignals()
{
...
void KimoraGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<KimoraUnits::Unit> units = KimoraUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const KimoraUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(KimoraUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (KimoraUnits::Unit u : KimoraUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(KimoraUnits::name(u)), this);
...
{
    setText(KimoraUnits::name(newUnits));
}
...
void KimoraGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=KMRBTC"));
