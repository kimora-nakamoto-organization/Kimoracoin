...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of KIMORACOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("KMR", type));
        BOOST_CHECK(!IsAssetNameValid("KMR", type));
        BOOST_CHECK(!IsAssetNameValid("KIMORACOIN", type));

        //- Versions of KIMORACOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("KMR.COIN", type));
        BOOST_CHECK(IsAssetNameValid("KMR_COIN", type));
        BOOST_CHECK(IsAssetNameValid("KMRSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERKMR", type));
        BOOST_CHECK(IsAssetNameValid("KMRSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEKMR", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_KMRS", type));
        BOOST_CHECK(IsAssetNameValid("SEKMROT", type));
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("KMR", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
