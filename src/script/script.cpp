...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** KMR START */
    case OP_KMR_ASSET              : return "OP_KMR_ASSET";
    /** KMR END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_KMR_ASSET) { // OP_KMR_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == KMR_R) { // Check to see if KMR starts at 27 ( this->size() < 105)
                if ((*this)[28] == KMR_V)
                    if ((*this)[29] == KMR_N)
                        index = 30;
            } else {
                if ((*this)[28] == KMR_R) // Check to see if KMR starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == KMR_V)
                        if ((*this)[30] == KMR_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == KMR_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == KMR_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == KMR_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == KMR_R) {
