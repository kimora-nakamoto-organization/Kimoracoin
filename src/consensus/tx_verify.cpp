...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, CAssetsCache* assetCache, bool fCheckDuplicateInputs, bool fMemPoolCheck, bool fCheckAssetDuplicate, bool fForceDuplicateCheck)
{
    ...
    for (const auto& txout : tx.vout)
    {
        ...
        // Make sure that all asset tx have a nValue of zero KMR
    ...
    if (AreAssetsDeployed()) {
        if (assetCache) {
            if (tx.IsNewAsset()) {
                ...
                // above transaction types.  Also fail if it contains OP_KMR_ASSET opcode but wasn't a valid script.
                for (auto out : tx.vout) {
                    int nType;
                    bool _isOwner;
                    if (out.scriptPubKey.IsAssetScript(nType, _isOwner)) {
                        ...
                        if (out.scriptPubKey.Find(OP_KMR_ASSET) > 0) {
