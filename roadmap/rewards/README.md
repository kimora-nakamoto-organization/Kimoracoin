...
Rewards, also sometimes called dividends, provides a way to send tokenized assets or KMR to token holders.  This can be used to reward shareholders with profits (denominated in KMR), or to reward membership holders, or to reward those that contributed the most to a shared project and earned special tokens.  
...
Example that rewards TRONCO holders with KMR:  
```reward 10000 KMR TRONCO```

## Reward calculation

First, the QTY of TARGET_TOKEN is calculated.  This is the total issuance, minus the qty held by the exception addresses.

Next, the reward calculation takes the qty to send.  This must be specified, and will usually be KMR.

PER_TOKEN_AMOUNT_IN_SATOSHIS = [QTY TO SEND_IN_SATOSHIS] / [QTY OF TARGET_TOKEN]

For KMR this must send an equal number of satoshis to every TARGET_TOKEN.  Remainder satoshis should be sent to the miners.
...
Example: 10 KMR to 3 TRONCO holders.  10 * 100,000,000 sats = 1,000,000,000 KMR sats.
PER_TOKEN_AMOUNT_IN_SATOSHIS = 1,000,000,000 / 3 = 333333333.33333333 (repeating) per TRONCO holder.  The remainder of .3333 (repeating) satoshis per holder will not be sent as an output, and therefore will be given to the miners.  This is 1 sat once multiplied by the 3 TRONCO holders.  Each TRONCO holder will receive exactly 333333333 KMR sats.
...
Select a token or KMR to send.
Set the QTY to send.
Select the target token
* If you are sending another token, you must have the owner token for the TARGET_TOKEN.
* If you are sending KMR, you do not need the owner token for the TARGET_TOKEN

GUI will show the exact amount being sent to each TARGET_TOKEN.  It will also calculate and show the remaining which must be returned as change.  If KMR is being sent, it will show the remainder that is being sent to the miners.
...
reward QTY [KMR|TOKEN] TARGET_TOKEN [exception address list]
...
reward 100 KMR TRONCO

Takes 100 KMR (10,000,000,000 sats)
RBQ5A9wYKcebZtTSrJ5E4bKgPRbNmr8M2H gets 10 KMR (10,000,000,000 KMR sats)
RPsCVwsq8Uf2dcUSXcYPzVnsAMZtAHw6sV gets 20 KMR (20,000,000,000 KMR sats)
RBp5woWDU8TRMz1TPeemyLxxLL3xsCnQgh gets 30 KMR (30,000,000,000 KMR sats)
RFMD7ZJzexAmiLA9BHxwFCPVeiuAgdVjcP gets 40 KMR (40,000,000,000 KMR sats)
```
...
reward 1000 KMR TRONCO ['RBQ5A9wYKcebZtTSrJ5E4bKgPRbNmr8M2H','RCqsnXo2Uc1tfNxwnFzkTYXfjKP21VX5ZD']

Takes 1000 KMR (100,000,000,000 sats)
RPsCVwsq8Uf2dcUSXcYPzVnsAMZtAHw6sV gets 100 KMR (10,000,000,000 KMR sats)
RBp5woWDU8TRMz1TPeemyLxxLL3xsCnQgh gets 300 KMR (30,000,000,000 KMR sats)
RFMD7ZJzexAmiLA9BHxwFCPVeiuAgdVjcP gets 600 KMR (60,000,000,000 KMR sats)
```
